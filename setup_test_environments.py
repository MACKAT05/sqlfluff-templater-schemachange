...
class PythonEnvironmentManager:
    ...
    def detect_python_versions(self) -> List[str]:
        ...
        for pattern in python_patterns:
            try:
                result = subprocess.run(
                    [pattern, "--version"], capture_output=True, text=True, check=True
                )
    ...
    def create_virtual_environment(
        self, python_executable: str, version: str
    ) -> Optional[Path]:
        ...
        try:
            ...
            logger.error(
                f"Failed to create virtual environment for Python {version}: {e}"
            )
            return None
    ...
    def run_sqlfluff_tests(self, venv_path: Path, version: str) -> Dict:
        ...
        try:
            ...
            if Path("examples").exists():
                ...
                try:
                    ...
                    if result.returncode != 0:
                        logger.warning(
                            f"SQLFluff linting found issues in Python {version}"
                        )
                    else:
                ...
                for sql_file in Path("examples/migrations").glob("*.sql"):
                    try:
                        ...
                        if result.returncode == 0:
                            logger.info(
                                f"✓ Rendered {sql_file.name} in Python {version}"
                            )
                        else:
                            logger.warning(
                                f"Failed to render {sql_file.name} in Python {version}"
                            )
            ...
            if self.temp_dir.exists():
                logger.info(
                    f"Running SQLFluff tests on temp examples (Python {version})"
                )
                        ...
                        if sql_files:
                            try:
                                ...
                                results["errors"].append(
                                    f"Linting {test_dir.name} timed out"
                                )
                ...
                try:
                    for sql_file in Path("migrations").glob("*.sql"):
                        try:
                            ...
                            if result.returncode == 0:
                                logger.info(
                                    f"✓ Schemachange rendered {sql_file.name} in Python {version}"
                                )
                            else:
                                logger.warning(
                                    f"Schemachange failed to render {sql_file.name} in Python {version}"
                                )

                        except subprocess.TimeoutExpired:
                            results["errors"].append(
                                f"Schemachange rendering {sql_file.name} timed out"
                            )
    ...
    def run_comprehensive_tests(self):
        ...
        for python_executable, version in python_versions:
            ...
            if venv_path:
                ...
                if self.install_packages(venv_path, version):
                    ...
                    schemachange_results = self.run_schemachange_tests(
                        venv_path, version
                    )
                    result["schemachange_results"] = schemachange_results

                    # Determine overall success
                    result["overall_success"] = sqlfluff_results.get(
                        "success", False
                    ) and schemachange_results.get("success", False)
                else:
        ...
        # Print summary
        successful = sum(
            1 for r in self.results.values() if r.get("overall_success", False)
        )
