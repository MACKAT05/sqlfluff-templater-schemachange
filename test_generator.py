...
class TestFileGenerator:
    ...
    def extract_named_files(self, content: str) -> Dict[str, str]:
        ...
        pattern = r'\*\*([^\*]+\.(sql|yml|yaml|ini|json|py|md|makefile))\*\*:\s*\n```[\w]*\n(.*?)\n```'
        ...
        pattern2 = r'Create `([^`]+\.(sql|yml|yaml|ini|json|py|md))`:\s*\n\n```[\w]*\n(.*?)\n```'
...
"""
        ...
        self._write_file("complex/migrations/V1.0.2__create_products.sql", child_template)
...
"""
        ...
        test_scenarios = [
            {
                ...
                "sql": "CREATE TABLE {{ database_name }}.{{ schema_name }}.{{ table_name }} (id INTEGER);"
            },
            {
                "name": "env_var_usage",
                ...
                "config": {
                    "vars": {
                        "database_name": "{{ env_var('TEST_DB', 'DEFAULT_DB') }}"
                    }
                },
    ...
    created_at TIMESTAMP
            ...
            },
            {
                ...
                "sql": "CREATE TABLE {{ sources.staging }}.cleaned_data AS SELECT * FROM {{ sources.raw }}.raw_data;"
            }
        ]
